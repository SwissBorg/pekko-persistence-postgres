...
import org.apache.pekko.persistence.postgres.query.dao.TestProbeReadJournalDao.JournalSequence
...
class TestProbeReadJournalDao(val probe: TestProbe) extends ReadJournalDao {
  ...
  /** Returns a Source of bytes for certain tag from an offset. The result is sorted by created time asc thus the offset
    * is relative to the creation time
    */
